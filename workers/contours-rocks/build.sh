...
fetch_dem() { # $1 = zdroj → DEM_VRT, DEM_GOT (čo sa NAOZAJ použilo)
  ...
  if [ "$rc" -eq 3 ]; then
    ...
    local how
    if [ "$src" = 'dmr5' ] && [ "$AREA_KEY_IN" != 'cely' ]; then
      how="workflow 'Dáta · DMR 5.0 z Drive' s area: $AREA_KEY_IN"
    elif [ "$src" = 'dmr5' ]; then
      how="workflow 'Dáta · DMR 5.0 z Drive' s area: cele_slovensko"
    else
      how="workflow 'Dáta · výškové modely do skladu' so zdrojom $src"
    fi
...
# v „Kontrola · workflowy a workery"; aj tento komentár preto tie dve zátvorky
# opisuje slovami.
