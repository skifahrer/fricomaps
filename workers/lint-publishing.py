...
def zdrojaky():
    ...
    return (sorted(glob.glob(".github/workflows/*.yml"))
            + sorted(glob.glob("workers/*.sh"))
            + sorted(glob.glob("workers/*.py")))
...
def bez_releasov():
    ...
    `workers/cleanup-actions.py` staré releasy MAŽE.
    """
    bad = 0
    ...
    for path in sorted(glob.glob("workers/*.py")):
        try:
        ...
        for job, jd in (d.get("jobs") or {}).items():
            for st in (jd or {}).get("steps") or []:
                ...
                print(f"::error file={path}::krok '{st.get('name', '?')}' "
                      ...
                      f"skladu na Drive (`workers/publish-results.sh`).")
                bad += 1
    return bad
