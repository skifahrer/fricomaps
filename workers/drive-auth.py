...
# VOLANIE API JE VEDĽA, v `drive-api.py`: jedna HTTP požiadavka s opakovaním,
# preklad odmietnutí na hlášku s návodom, `files.delete` a otázka „aké rozsahy
# ten token naozaj dostal". Tento súbor sa pýta na niečo iné: KTO sme – odkiaľ
# sa berú prihlasovacie údaje, ako sa access token obnovuje a ako sa jednorazovo
# prihlásiť. (Rozdelené preto, že spolu to malo 894 riadkov – pravidlo 5.)
#
# Mená sa tu vystavujú ĎALEJ, takže ostatné workery ich vidia na tom istom
# mieste ako doteraz (`auth.api_get`, `auth.AuthError`, `auth.scope_hint`) a
# nemuseli sa prepisovať.
api = load("drive_api", "drive-api.py")
