...
# `workers/features.yml`, nech sa obe menia na jednom mieste. Bez predfiltra by
...
# maxzoomom, Planetiler zahodí bez slova. Viď workers/features.yml.
TOPZ=$(grep -oE 'min_zoom: [0-9]+' workers/features.yml \
       | grep -oE '[0-9]+' | sort -n | tail -1)
if [ "${TOPZ:-0}" -gt "$FZ" ]; then
  echo "::warning::workers/features.yml má triedy s min_zoom až ${TOPZ}, ale dlaždice idú po z${FZ} – tie sa do nich vôbec nedostanú. Zdvihni features_maxzoom na ${TOPZ}, alebo tým triedam zníž min_zoom."
fi

T_PM=$(date +%s)
OUT="_site/tiles/${REGION_KEY}-features.pmtiles"
java -Xmx4g -jar planetiler.jar generate-custom \
  --schema=workers/features.yml \
  --output="$OUT" \
